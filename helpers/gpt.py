...
def parse_string_gpt(panel):
    ...
    completion = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=format_message(panel)
    )

    
    return completion.choices[0].message.content.strip()

def format_message(panel):
    return [
        {"role": "system", "content": """
...
1. **Extract Street Information:**
   ...
   - Look for mentions of province/state in the header text which start like this Վթարային ջրանջատում 'state' 'province' վարչական շրջանում մայիսի 31-ին
«Վեոլիա Ջուր» ընկեր
...
7. Your output should be only JSON nothing else and it should follow the structure mentioned above

        """},
        {"role": "system", "content": "here is the message: " + panel['text']}
    ]